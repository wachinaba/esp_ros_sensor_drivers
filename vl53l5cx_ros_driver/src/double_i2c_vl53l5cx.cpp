...
void setup() {
  ...
  if (myImager1.begin() == false) {  // I2C_RST does not reset the I2C address,
...
void loop() {
  ...
  if (myImager1.isDataReady() == true) {
    if (myImager1.getRangingData(
            &measurementData1))  // Read distance data into array
    {
  ...
  if (myImager2.isDataReady() == true) {
    if (myImager2.getRangingData(
            &measurementData2))  // Read distance data into array
    {
      // The ST library returns the data transposed from zone mapping shown in
      // datasheet Pretty-print data with increasing y, decreasing x to reflect
      // reality
